...
const resolvedItems = computed(() => {
  ...
  return props.items.map((it) => {
    if (it && typeof it === 'object') return { ...it, src: resolveImg(it.src) }
...
<style scoped>
.slideshow { display: grid; gap: .35rem; border-radius: 16px; background: transparent; box-shadow: 0 10px 30px rgb(0 0 0 / .25); overflow: hidden; }
.viewport { position: relative; width: 100%; padding-bottom: var(--ratio, 56.25%); background: var(--vp-c-bg-soft); overflow: hidden; border-radius: 16px; }
.track { position: absolute; inset: 0; display: flex; transition: transform .35s ease; will-change: transform; }
.slide { min-width: 100%; height: 100%; position: relative; }
.slide img { display: block; width: 100%; height: 100%; object-fit: cover; }
.nav { position: absolute; top: 50%; transform: translateY(-50%); width: 44px; height: 44px; border-radius: 50%; border: none; background: rgba(20,20,20,.7); color: #fff; font-size: 28px; line-height: 1; display: grid; place-items: center; cursor: pointer; z-index: 4; }
.nav.left  { left: 10px; }
.nav.right { right: 10px; }
.overlay-caption { position: absolute; left: .75rem; bottom: 2.1rem; background: rgba(0,0,0,.35); color: #fff; padding: .5rem 1rem; border-radius: .1rem; font-size: .95rem; opacity: .95; z-index: 3; max-width: 80%; box-shadow: 0 2px 8px rgba(0,0,0,.18); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); }
.dots { position: absolute; left: 0; right: 0; bottom: .6rem; display: flex; justify-content: center; gap: .4rem; z-index: 5; pointer-events: auto; }
.dot { width: 8px; height: 8px; border-radius: 999px; background: rgba(255,255,255,.35); border: 0; padding: 0; }
