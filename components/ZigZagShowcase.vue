    ...
    >
      <div class="media">
        <ImageSlideshow
          ...
          pause-on-hover
        />
        <img v-else :src="resolveImg(s.src)" :alt="s.alt || 'image'" loading="lazy" />
      </div>
...
.row { display: grid; gap: 2rem; align-items: center; grid-template-columns: 1fr; }
@media (min-width: 1024px) {
  .row { grid-template-columns: minmax(0, 1.1fr) minmax(0, 0.9fr); }
  .row .media { order: 1; }
  .row .copy  { order: 2; justify-self: start; text-align: left; }
  .row.reverse .media { order: 2; }
  .row.reverse .copy  { order: 1; justify-self: end; text-align: right; }
}
.media { border-radius: 16px; overflow: hidden; background: transparent; box-shadow: 0 10px 30px rgb(0 0 0 / 0.25); }
.media :deep(img), .media :deep(video) { display: block; width: 100%; height: 100%; }
.copy { max-width: 65ch; }
